$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'flow/nodeinstance/list/done',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		getNextPeople: function(formId,nodeCode){
			$.get(baseURL + "flow/executerole/select/"+formId+"?nodeCode="+nodeCode, function(r){
		...
		nextPeople: function(){
            vm.getNextPeople(vm.nodeInstance.formId,vm.nodeInstance.nodeCode);
			layer.open({
                type: 1,
                offset: '50px',
                skin: 'layui-layer-molv',
